 ...
 ***********************************************/
 ...
 UE3::UE3( const std::string &name ) : Atomic( name )
 , In( addInputPort( "In" ) )
 ...
 Model &UE3::externalFunction( const ExternalMessage &msg )
  {
 ...
 Model &UE3::outputFunction( const InternalMessage &msg )
 {
	if (state == RecPack)
	   {
 ...
 Model &UE3::internalFunction( const InternalMessage & )
 {
	switch (state){
				    case Idle:
				    	       //Temp = 30000 + (SentPacketNum * 2);  // Src = 3 , Des =  ==> 3 0000
				    	       //state = SendPack;
