 ...
 ***********************************************/
 ...
 UE2::UE2( const std::string &name ) : Atomic( name )
 , In( addInputPort( "In" ) )
 ...
 Model &UE2::externalFunction( const ExternalMessage &msg )
  {
 ...
 Model &UE2::outputFunction( const InternalMessage &msg )
 {
	if (state == RecPack)
	   {
 ...
 Model &UE2::internalFunction( const InternalMessage & )
 {
	switch (state){
				    case Idle:
				    	       Temp = 230000 + (SentPacketNum * 2);  // Src = 2 , Des = 3 ==> 230000
				    	       state = SendPack;
